	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Helloo, Sejam Bem Vindo (a) @${num.split('@')[0]}\nCaso queira ver meus comandos... Digite /comandos *${mdata.subject}*`
				...
				teks = `O @${num.split('@')[0]}Ja foi tarde kkkjjjkk🐊`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: '[❗] Este comando só pode ser usado pelo ザ・ ᴅɪɢᴜɪɴʜᴏ『🍜』 ❌',
					admin: '[❗] Vai se Fuder Membro Comum🖐🏻 Este comando só pode ser usado por algum administrador!! ❌',
					Badmin: '[❗] Eu nem sou administrador Fdp, você acha que eu sou magico?❌',
		...
		client.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
               ...
               break
				case 'demote':
					...
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marca a pessoa que você quer tornar um membro Comum!')
					...
					if (mentioned.length > 1) {
						...
						mentions(`Comando aceito, @${mentioned[0].split('@')[0]}\n Foi reduzido para membro comum _*${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
                  ...
                  case 'promote':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marca a pessoa que você quer transformar como dono da porra toda')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Rapaziada agora fudeu, nosso (a) novo administrador (a) é: :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Rapaziada agora fudeu, nosso (a) novo administrador (a) é: : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
	      ...
	      tels = body.slice(6)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'chord':
					if (args.length < 1) return reply('onde está o título da música, Viadinho?')
                                        /*if (!isDaftar) return reply(mess.only.daftarB)*/
					tels = body.slice(7)					
					...
					break
				case 'lirik':
					if (args.length < 1) return reply('Onde estão as letras, Viadinhoa?')
                                        /*if (!isDaftar) return reply(mess.only.daftarB)*/
					tels = body.slice(7)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Coloca o numero da pessoa que você quer adicionar, De preferência o de sua mãe!!!')
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Banido kkkkjjkkkjjk🐊🐊🐊🐊🐊 : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
				...
				case 'bugreport':
				client.updatePresence(from, Presence.composing) 
				if (args.length < 1) return reply('Qual é o bug, mana?')
					tek = body.slice(10)
					bug = {
